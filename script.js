...
function addSblogs(){
  ...
  function addBlogs(date,tag,desc,logo,url,count){
    ...
    sblog.innerHTML = '<div class="left"><div id="date">' + date + '</div><div id="tag"><span id="s1">'+ tag[0]+ '</span> <span id="s2">' + tag[1] + '</span></div><div id="desc">'+ desc + '</div><a id="url" href=" '+ url +' ">Read...</a></div><div class="right"><img id="logo" src=" ' + logo + ' " /></div>';
...
function mblog(url) {
  ...
  function addMblog(data) {
    b.innerHTML = '<div id="date">' + data.date + '</div><div id="tag"><span id="s1">' + data.tag[0] + '</span> <span id="s2">' + data.tag[1] + '</span></div><div id="mcontent">' + data.content + '</div></br><a href="" id="back">Back...</a>';
